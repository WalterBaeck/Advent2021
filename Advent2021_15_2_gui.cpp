...
int main(int Arguments, char* Argument[])
{
	...
	NrOfLines *= 5;
	InputLen *= 5;
  int RedMax = (InputLen + NrOfLines)*4;

	Gui gui("AdventOfCode2021 15 2", 2*InputLen, 2*NrOfLines);
#if 0
	while (gui.alive)
		gui.process();
#endif
	...
	NewX = (int*)malloc(10000 * sizeof(int));
	NewY = (int*)malloc(10000 * sizeof(int));
	ExtraX = (int*)malloc(10000 * sizeof(int));
	ExtraY = (int*)malloc(10000 * sizeof(int));
	int NrOfNew,NewNr;
	...
	Cost[0][0] = 1;
	X=0; Y=0;
	gui.drawPixel(ePermDC, XY(2*X  , 2*Y  ), 0,0,0);
	gui.drawPixel(ePermDC, XY(2*X+1, 2*Y  ), 0,0,0);
	gui.drawPixel(ePermDC, XY(2*X  , 2*Y+1), 0,0,0);
	gui.drawPixel(ePermDC, XY(2*X+1, 2*Y+1), 0,0,0);
	...
	for (; NrOfNew>0;)
	{
		...
		for (NewNr=0; NewNr<NrOfNew; NewNr++)
		{
...
#if 1
			gui.drawPixel(eTempDC, XY(2*X  , 2*Y  ), 240,224,176);
			gui.drawPixel(eTempDC, XY(2*X+1, 2*Y  ), 240,224,176);
			gui.drawPixel(eTempDC, XY(2*X  , 2*Y+1), 240,224,176);
			gui.drawPixel(eTempDC, XY(2*X+1, 2*Y+1), 240,224,176);
#endif
			...
			for (ChoiceNr=1; ChoiceNr<=4; ChoiceNr++)
			{
...
#if 1
				gui.drawPixel(eTempDC, XY(2*DestX  , 2*DestY  ), 0,160,0);
				gui.drawPixel(eTempDC, XY(2*DestX+1, 2*DestY  ), 0,160,0);
				gui.drawPixel(eTempDC, XY(2*DestX  , 2*DestY+1), 0,160,0);
				gui.drawPixel(eTempDC, XY(2*DestX+1, 2*DestY+1), 0,160,0);
#endif
				...
				unsigned char red = (unsigned char)((Cost[X][Y]<<8) / RedMax);
#if 1
				gui.drawPixel(ePermDC, XY(2*DestX  , 2*DestY  ), red, 0, 0);
				gui.drawPixel(ePermDC, XY(2*DestX+1, 2*DestY  ), red, 0, 0);
				gui.drawPixel(ePermDC, XY(2*DestX  , 2*DestY+1), red, 0, 0);
				gui.drawPixel(ePermDC, XY(2*DestX+1, 2*DestY+1), red, 0, 0);
#endif
			} /* for (ChoiceNr) */
		} /* for (NewNr) */
