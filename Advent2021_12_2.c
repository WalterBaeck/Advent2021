...
printf("Created Node[%d]=%s%s\n", NodeNr, Node[NodeNr].Name, (Node[NodeNr].SingleUse)?" SingleUse":"");
	...
	// Allow a Joker to visitone particular small Node twice
	int JokerUsed=0;
	int JokerFrom=-1,JokerTo=-1;
...
printf("Iteration starts Step[%d] from Node[%d]=%s beyond Choice %d out of %d\n",
		...
		for (ChoiceNr++; ChoiceNr<NrOfChoices; ChoiceNr++)
		{
			...
			if ((Node[DestNode].SingleUse) && (Node[DestNode].Used))
			{
				...
				// However, the Joker might allow this Choice after all
				if ((JokerUsed) || (DestNode == StartNode) || (DestNode == EndNode))
					continue;
				else
				{
					// Register the one-time use of the Joker
					JokerUsed = 1;
					JokerFrom = NodeNr;
					JokerTo = DestNode;
				}
		...
		if (ChoiceNr >= NrOfChoices)
		{
			...
			if (Node[NodeNr].SingleUse)
			{
				// ...but wait, perhaps this happens to be the Step that uses the Joker
				if ((JokerUsed) && (JokerFrom == NodeAtStep[NrOfSteps-1]) && (JokerTo == NodeNr))
				{
					// Then we clear the Joker, but NOT the Used status
					JokerUsed = 0;
					JokerFrom = -1;
					JokerTo   = -1;
				}
		...
		{
			NrOfPaths++;
for (StepNr=0; StepNr<NrOfSteps; StepNr++)  printf("%s,", Node[NodeAtStep[StepNr]].Name);
printf("%s\n", Node[DestNode].Name);
