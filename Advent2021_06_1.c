...
int main(int Arguments, char* Argument[])
{
	...
	for (NrOfDays=1; NrOfDays<=80; NrOfDays++)
	{
		...
		memcpy(Group, NewGroup, 9*sizeof(int));
		// Compute the sum of all Fish
		SumFish = 0;
		for (GroupNr=0; GroupNr<9; GroupNr++)
			SumFish += Group[GroupNr];
		if ((NrOfDays<=18) || (NrOfDays==80))
			printf("After %2d days, SumFish is %d\n", NrOfDays, SumFish);
	}
	return 0;
}		
