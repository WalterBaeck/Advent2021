...
printf("\n");
				...
				for (DigitNr=0; DigitNr<10; DigitNr++)
				{
					...
					for (GarbledNr=0; GarbledNr<10; GarbledNr++)
					{
						...
						if (SegmentNr >= 7)
						{
printf("Seen[%d] is Digit[%d] in this permutation\n", DigitNr, GarbledNr);
				...
				if (DigitNr >= 10)
				{
					...
					for (DigitNr=0; DigitNr<4; DigitNr++)
					{
						...
						for (GarbledNr=0; GarbledNr<10; GarbledNr++)
						{
							...
							{
								printf(" %d", GarbledNr);
								if ((GarbledNr==1) || (GarbledNr==4) || (GarbledNr==7) || (GarbledNr==8))
									NrOf1478++;
								break;
							}
						}
	...
	printf("%d lines scanned - NrOf1478 is %d\n", NrOfLines, NrOf1478);
